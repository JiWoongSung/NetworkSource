...
int main(int argc, char *argv[])
{
    ...
    int portno; // 포트번호저장
    struct sockaddr_in6 serv_addr; // ipv6정보를 담을 구조체 
    ...
    sockfd = socket(PF_INET6, SOCK_STREAM, 0); //새로운 소켓 생성(IPv6,TCP통신) 
    if (sockfd < 0)  //소켓생성 실패면 오류 
        error("ERROR opening socket");
    
    server = gethostbyname2(argv[1],AF_INET6); //argv[1]의 문자열(DNS)를 IP 주소, 주소 유형, 주소 길이 등의 정보가 들어있는 struct hostent로 반환
    ...
    serv_addr.sin6_family = AF_INET6; //인터넷프로토콜 설정 
    serv_addr.sin6_port = htons(portno); // 포트번호 설정
    serv_addr.sin6_flowinfo = 0; //IPv6 flow information
